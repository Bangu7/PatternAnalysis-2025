# Using a Siamese neural network to for classification on ICIS 2020 dataset.
...
A Siamese network is a neural network designed to compare two input samples, in this case, image pairs. The network learns to embed images into a feature space such that similar images are put close together and dissimilar further apart. The network contains two identical sub-networks that share weights. The output is a pair of embeddings that can be compared by a loss function. One of the common ones being contrastive loss. This model is represented in the following, figure 1:

*Figure 1: Siamese Network*
![Siamese Network](./images/siamese_net.png)

In this project the ResNet-18 model was used as the base model, a widely used deep CNN. ResNet-18 introduces residual connections to standard deep CNN, which allows for deeper layers and better gradient flow. This model was used with its Default weights that were trained on IMAGENET1K. (Resnet18 — Torchvision Main Documentation, 2024)
...
To determine the stopping metric, AUC-ROC was used. This measures represents the probability given a random positive and negative sample that it will rank the positive higher than the negative. (Google Developers, 2019)

### Optimiser

Both models used the same optimiser Adam (Adaptive Moment Estimation) with a learning rate of 1e-4 for Siamese and 5e-5 for Classifier, other rates were tested but would make training unstable or model would greatly overfit training data.
...
The dataset was processed into training, validation and testing subsets of 70/10/20 respectively. This allowed for majority of the data to be used in training while, still enough to effectively evaluate the models performance.
...
In contrast, the Classifier has a more steady decay as training and validation loss decrease together. This indicates it is generalising well and less overfitting. The model appears to slowly flatten out indicating little reward for more epochs.
...
(Awan, 2023) t-SNE is an unsupervised non-linear dimensionality reduction technique. The plot it generates gives us an idea of how the Siamese Network embeds the images. There is indication of separation between the two classes, however there is still a fair bit of overlap. Particularly with benign being in what appears to be a malignant cluster on the left. These benign lesions likely have similar features to the malignants making it harder for the model to distinguish between.
...
- This model was trained locally on a gtx1060, thus, with better equipment more training could be provided with more thorough hyperparameter tuning.
